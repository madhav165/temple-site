...
arr3=[]
dft = pd.read_csv('temples.csv')
temple_urls = dft.loc[dft['url']!='https://www.bharattemples.com/tamil-nadu']['temple_url'].tolist()

for temple_url in temple_urls:
    print(temple_url)
    resp = requests.get(temple_url)
    resp.status_code
    soup = BeautifulSoup(resp.content)
    body = soup.body
    contents = body.find('div', {'class': 'entry-content'}).find_all('p')
    content = '\n'.join([c.text for c in contents])
    arr3.append([temple_url, content])
    time.sleep(1)
pd.DataFrame(arr3).to_csv('non_tn_temples_v0.csv', index=False)
