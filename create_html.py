...
for index,row in df1.iterrows():
    ...
    state_description = re.sub(r'[^\w_. -]', ' ', state_description).replace('  ', ' ')
    state_description = state_description.strip().encode('latin-1', 'replace').decode('latin-1')
    ...
    if not os.path.isdir(os.path.join('India', state_name)):
        ...
        <html>
        <head>
        <title>{state_name} Temples</title>
        </head>
        <body>
        <h1>{state_name}</h1>
        ...
        '''
    with open(os.path.join('India', state_name, state_name.replace(',','_').replace(' ','_')+'.html'), 'w') as f:
        f.write(html)
    ...
    for index2, row2 in df3.iterrows():
        ...
        <html>
        <head>
        <title>{temple_name} Temples</title>
        </head>
        <body>
        <h1>{temple_name}</h1>
        <p>{temple_details}</p>
        ...
        '''
        with open(os.path.join('India', state_name, temple_name.replace(',','_').replace(' ','_')+'.html'), 'w') as f:
            f.write(html)
